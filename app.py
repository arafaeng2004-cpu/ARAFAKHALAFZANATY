...
# ------------------------------------------------------------------------------
st.sidebar.markdown("### 🏢 **EmaratBuild OS v4.5**")
...
# ==============================================================================
    ...
    st.caption(f"المرجع الرقابي: بلدية {pt['emirate']} | حالة جهد التربة: {pt['sbc_status']}")
    ...
    with ci4:
        pt["bua_target"] = st.number_input("مسطح البناء الإجمالي BUA (م²):", 100.0, 15000.0, float(pt["bua_target"]), 25.0, key="gf_bua_inp")
    ...
    with tabs_gf[0]:
        st.subheader("المسقط المعماري التنفيذي المعتمد - الطابق الأرضي (Double-Line CAD Plan)")
    ...
    with tabs_gf[2]:
        ...
        ax_st2.set_xlim(-3, st_span_gf + 3); ax_st2.set_ylim(-1, ridge_h_gf + 2); ax_st2.axis('off')
        st.pyplot(fig_st2)
    ...
    with tabs_gf[3]:
        ...
        boq_data = [
            ...
            {"الكود": "02.00", "البند": "الحفر لزوم التأسيس وردم الدفان على طبقات", "الوحدة": "م³", "الكمية": round(pt["bua_target"] * 1.6, 1), "السعر (AED)": 12.0},
            ...
            {"الكود": "04.00", "البند": "طابوق إسمنتي عازل 20 سم ومفرغ للقواطع الداخلية", "الوحدة": "م²", "الكمية": round(pt["bua_target"] * 2.2, 1), "السعر (AED)": 110.0},
            ...
            {"الكود": "07.00", "البند": "الواجهات الزجاجية المزدوجة والألمنيوم واللوفرز", "الوحدة": "م²", "الكمية": round(pt["bua_target"] * 0.22, 1), "السعر (AED)": round(750.0 * mult, 1)},
